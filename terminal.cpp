...
void WriteToTerminal(std::string toAppende, ALLEGRO_COLOR colourToAppend)
{
    ...
    for(std::string::iterator it = toAppend.begin(); toAppend.begin() != toAppend.end(); ++it)
    {
        ...
        lineRemainingWidth = TERMINAL_MAX_WIDTH - s_al_get_text_width(robotoSlabFont,terminalLine[0]);
        ...
        if(*it == ' ' || *it == '\0')
        {
            ...
            if(s_al_get_text_width(robotoSlabFont,toAppend) < lineRemainingWidth)
            {
